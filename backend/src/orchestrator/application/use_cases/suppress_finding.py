...
pure `domain.services.finding_transitions.suppress` FSM. Powers
...
from orchestrator.domain.services.finding_transitions import suppress
...
async def suppress_finding(
    ...
    """Transition `finding_id` to `SUPPRESSED`.

    `OPEN -> SUPPRESSED` persists via `FindingPort.update_status`.
    `SUPPRESSED -> SUPPRESSED` is an idempotent no-op: the current finding is
    returned WITHOUT a write (no `updated_at` bump).
    ...
    resulting_status = suppress(finding.status)
