"""`deactivate_scan_target` use case — idempotent soft-delete (dast-scanner
PR6, mirrors `test_deactivate_repository.py`)."""
...
def test_deactivate_scan_target_raises_not_found_for_absent_id() -> None:
    target_port = _FakeScanTargetRepository()

    with pytest.raises(ScanTargetNotFoundError):
        asyncio.run(deactivate_scan_target(target_port, uuid.uuid4()))


def test_deactivate_scan_target_sets_is_active_false() -> None:
    target_port = _FakeScanTargetRepository()
    target = _make_target()
    target_port.seed(target)

    asyncio.run(deactivate_scan_target(target_port, target.id))

    assert target.is_active is False


def test_deactivate_scan_target_is_idempotent_for_already_inactive_target() -> None:
    target_port = _FakeScanTargetRepository()
