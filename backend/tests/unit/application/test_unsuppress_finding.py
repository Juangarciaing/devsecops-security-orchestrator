"""`unsuppress_finding` use case — transitions a `Finding` back to `OPEN` via the
`domain.services.finding_transitions` FSM. Powers `POST /findings/{id}/unsuppress`.
"""
...
def test_unsuppress_finding_raises_not_found_for_absent_finding() -> None:
    finding_port = _FakeFindingRepository()

    with pytest.raises(FindingNotFoundError):
        asyncio.run(unsuppress_finding(finding_port, uuid.uuid4(), UserRole.ADMIN))


def test_unsuppress_finding_transitions_suppressed_to_open_and_writes() -> None:
    finding_port = _FakeFindingRepository()
    ...
    assert finding_port.update_status_calls == [(finding.id, FindingStatus.OPEN)]


def test_unsuppress_finding_already_open_is_idempotent_no_write() -> None:
    finding_port = _FakeFindingRepository()
...
def test_unsuppress_finding_raises_illegal_transition_for_resolved_or_false_positive(
    ...
    with pytest.raises(IllegalStatusTransitionError):
        asyncio.run(unsuppress_finding(finding_port, finding.id, UserRole.ADMIN))

    assert finding_port.update_status_calls == []


def test_unsuppress_finding_redacts_sensitive_fields_for_member() -> None:
    finding_port = _FakeFindingRepository()
    finding = _make_finding(status=FindingStatus.SUPPRESSED)
    finding_port.seed(finding)
...
def test_unsuppress_finding_leaves_sensitive_fields_intact_for_admin() -> None:
    finding_port = _FakeFindingRepository()
    finding = _make_finding(status=FindingStatus.SUPPRESSED)
    finding_port.seed(finding)
