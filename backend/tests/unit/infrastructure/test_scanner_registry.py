...
factory (Module 7 D2, tasks 1.3-1.4; Module 11 adds the `SCA` and `SAST`
registrations, task 3.1).
...
def test_get_adapter_resolves_ast_sast_adapter_for_sast() -> None:
    from orchestrator.infrastructure.scanners.ast_sast_adapter import AstSastAdapter
    from orchestrator.infrastructure.scanners.registry import get_adapter

    adapter = get_adapter(ScannerType.SAST, FakeContainerRunner(), _settings())

    assert isinstance(adapter, AstSastAdapter)


def test_get_adapter_returns_an_ast_sast_adapter_that_implements_the_port_contract() -> None:
    from orchestrator.domain.ports.scanner_adapter_port import ScannerAdapterPort
    from orchestrator.infrastructure.scanners.registry import get_adapter
